module.exports = {
    ...
    async executar(socket, msg, args) {
        ...
        if (!(participantes.find(p => p.id === meuJid)?.admin?.includes('admin'))) {
            return await socket.sendMessage(deOnde, { text: '❌ Thánatos necessita de privilégios administrativos para revogar poderes.' });
        }

        const remetente = msg.key.participant || msg.key.remoteJid;
        if (!(participantes.find(p => p.id === remetente)?.admin?.includes('admin'))) {
            return await socket.sendMessage(deOnde, { text: '❌ Apenas administradores podem retirar o poder de outrem.' });
        }
        ...
        if (!alvo) return await socket.sendMessage(deOnde, { text: '❌ Indique quem deseja rebaixar marcando a pessoa ou respondendo à sua mensagem.' });

        try {
            await socket.groupParticipantsUpdate(deOnde, [alvo], 'demote');
            await socket.sendMessage(deOnde, { text: `⚖️ *QUEDA DO PODER.*\n\n@${alvo.split('@')[0]} perdeu seus privilégios e retornou ao plano dos mortais comuns.`, mentions: [alvo] });
        } catch (erro) {
            await socket.sendMessage(deOnde, { text: '❌ Falha ao tentar rebaixar o usuário.' });
        }
    }
};